...
// check explicit number of flops ...
// long count = 0;
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    long icount = 2*m*n*k*NREPEATS;
    double time = t.toc();
    double flops = icount / 1.e9 / time; 
    double read_write = 4*m*k*n*NREPEATS*sizeof(double);
    double bandwidth = read_write / 1.e9 / time;
    printf("%10ld %10f %10f %10f\n", p, time, flops, bandwidth);
    // printf("count = %10ld, icount = %10ld \n \n", count, icount);
